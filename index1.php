...
<!--            <textarea name="comment" cols="30" rows="5" placeholder="Оставить отзыв"></textarea>-->
        ...
        <? if ($_POST['comment']. $_POST['name']) {
            ...
            $name = $_POST['name'];
            $time = date("H:i:s");
            $connection->query("INSERT INTO `comment` (`commentary`,`name`,`date`) VALUES ('$comment','$name','$time')");
        }
        $commentOfUser = $connection->query('SELECT * FROM `comment`');
        foreach ($commentOfUser as $comments) {
            ?>
            <div class="comment_block" style="">
                <?='№'.$comments['id'].' '.$comments['name'].'   '.'Оставил отзыв в:'.' '.$comments['date'].'-' .$comments['commentary']?>
            </div>
        <? }?>
    </div><!--//main-body-->
</div>
